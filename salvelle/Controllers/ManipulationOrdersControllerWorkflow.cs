...
/// <summary>
/// Extens�o do ManipulationOrdersController com endpoints de workflow
/// VERS�O CORRIGIDA - Adaptada aos models existentes (Batch, ManipulationOrder, StockMovement)
/// </summary>
public partial class ManipulationOrdersController
{
    ...
    public async Task<ActionResult<ApiResponse>> StartSeparacao(Guid id, [FromBody] StartSeparacaoDto dto)
    {
        ...
        if (order == null)
            return NotFound(ApiResponse.ErrorResponse("Ordem n�o encontrada"));

        if (order.Status != "PENDENTE")
            return BadRequest(ApiResponse.ErrorResponse("Ordem deve estar PENDENTE para iniciar separa��o"));
        ...
        foreach (var item in dto.Items)
        {
            ...
            if (batch == null)
                return BadRequest(ApiResponse.ErrorResponse($"Lote {item.BatchNumber} n�o encontrado"));

            if (batch.CurrentQuantity < item.QuantitySeparated)
                return BadRequest(ApiResponse.ErrorResponse(
                    $"Quantidade insuficiente no lote {item.BatchNumber}. Dispon�vel: {batch.CurrentQuantity}"));
        }
        ...
        return Ok(ApiResponse.SuccessResponse("Separa��o de materiais conclu�da com sucesso"));
    }
    ...
    public async Task<ActionResult<ApiResponse<ManipulationStepDto>>> StartPesagem(Guid id, [FromBody] StartPesagemDto dto)
    {
        ...
        if (order == null)
            return NotFound(ApiResponse<ManipulationStepDto>.ErrorResponse("Ordem n�o encontrada"));
        ...
        return Ok(ApiResponse<ManipulationStepDto>.SuccessResponse(null!, "Pesagem conclu�da com sucesso"));
    }
    ...
    public async Task<ActionResult<ApiResponse>> CheckPesagem(Guid id, [FromBody] CheckPesagemDto dto)
    {
        ...
        if (step == null)
            return NotFound(ApiResponse.ErrorResponse("Etapa de pesagem n�o encontrada"));
    ...
    public async Task<ActionResult<ApiResponse>> StartMistura(Guid id, [FromBody] StartMisturaDto dto)
    {
        ...
        return Ok(ApiResponse.SuccessResponse("Mistura conclu�da com sucesso"));
    }
    ...
    public async Task<ActionResult<ApiResponse>> StartEnvase(Guid id, [FromBody] StartEnvaseDto dto)
    {
        ...
        return Ok(ApiResponse.SuccessResponse("Envase conclu�do com sucesso"));
    }
    ...
    public async Task<ActionResult<ApiResponse>> StartRotulagem(Guid id, [FromBody] StartRotulagemDto dto)
    {
        ...
        return Ok(ApiResponse.SuccessResponse("Rotulagem conclu�da com sucesso"));
    }

    // ===================================================================
    // ETAPA 5: CONFER�NCIA FINAL
    // ===================================================================
    ...
    public async Task<ActionResult<ApiResponse>> StartConferencia(Guid id, [FromBody] StartConferenciaDto dto)
    {
        ...
        return Ok(ApiResponse.SuccessResponse(
            dto.ApprovedByPharmacist ? "Confer�ncia aprovada - Ordem finalizada" : "Confer�ncia registrada"));
    }
    ...
    public async Task<ActionResult<ApiResponse>> DeletePhoto(Guid photoId)
    {
        ...
        if (photo == null)
            return NotFound(ApiResponse.ErrorResponse("Foto n�o encontrada"));

        // Foto não tem EstablishmentId: posse herdada da ordem. Fecha IDOR cross-tenant.
    ...
    // ===================================================================
    // ETAPA 6: APROVA��O FINAL DO FARMAC�UTICO
    // ===================================================================
    ...
    public async Task<ActionResult<ApiResponse>> StartAprovacao(Guid id, [FromBody] StartAprovacaoDto dto)
    {
        ...
        if (conferenciaStep == null || conferenciaStep.Status != "CONCLUIDA")
            return BadRequest(ApiResponse.ErrorResponse("Confer�ncia deve ser conclu�da antes"));
        ...
        if (pharmacist == null)
            return BadRequest(ApiResponse.ErrorResponse("Farmac�utico n�o encontrado"));

        if (string.IsNullOrEmpty(dto.PharmacistCRF))
            return BadRequest(ApiResponse.ErrorResponse("CRF obrigat�rio"));
        ...
        // Usar QualityNotes para motivo de rejei��o (ManipulationOrder n�o tem RejectionReason)
        ...
        return Ok(ApiResponse.SuccessResponse(
            dto.Approved ? "Aprovado pelo farmac�utico" : $"Rejeitado: {dto.RejectionReason}"));
    }
    ...
    public async Task<ActionResult<ApiResponse>> StartExpedicao(Guid id, [FromBody] StartExpedicaoDto dto)
    {
                ...
                foreach (var item in separacaoData.Items)
                {
                    ...
                    if (batch != null)
                    {
                        ...
                        var movement = new StockMovement
                        {
                            ...
                            Reason = $"Baixa expedi��o - OM {order.OrderNumber}",
        ...
        return Ok(ApiResponse.SuccessResponse("Expedi��o registrada. Ordem finalizada."));
    }
    ...
    public async Task<ActionResult<ApiResponse>> ConfirmDelivery(Guid id, [FromBody] ConfirmDeliveryDto dto)
    {
        ...
        if (step == null)
            return NotFound(ApiResponse.ErrorResponse("Expedi��o n�o encontrada"));
    ...
    public async Task<ActionResult<ApiResponse<WorkflowDashboardDto>>> GetWorkflowDashboard()
    {
        ...
        // Usar ExpectedDate (ManipulationOrder n�o tem EstimatedCompletionDate)
